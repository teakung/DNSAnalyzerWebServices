...
router.get('/summary/:startTime/:endTime', function (req, res, next) {
  ...
  body: {
    ...
    var summaryStartTime = req.params.startTime
    var summaryEndTime = req.params.endTime
    var result = []
    for(var i = 0 ;i <= days-1 ; i++){
    	for(var j=0; j <= 23 ;j++){
    		if(hits[(i*24)+j] != undefined){
    			...
    			result[(i*24)+j] = [parseInt(j),parseInt(i),0,0]
    		}
    		//console.log((i*24)+j);
    	}
    }
...
/*    "sort" : [
    ...
    //var hits = resp.hits.hits;
    var hits = resp.aggregations.Nest.Grouping.buckets;
    //console.log(resp.aggregations.Nest.Grouping.buckets)
    //var hits = resp;

    var result = []
    for(a in hits){
      result.push([hits[a].key,hits[a].sumcount.value])
    }
